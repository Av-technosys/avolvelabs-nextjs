...
void main() {
  ...
  vec3 lightBackdrop = vec3(0.941176, 0.972549, 1.0);
  vec3 backdrop = mix(vec3(0.070588, 0.058824, 0.090196), lightBackdrop, step(0.5, uLightMode));
  ...
  if (uLightMode > 0.5) {
    ...
    float fibers = pow(smoothstep(0.11, 0.96, fiberField) * edgeFade, 1.52);
    float atmosphere = (center * 0.028 + cloud * 0.018) * edgeFade;
    vec3 fiberInk = mix(backdrop, uLineColor, 0.5);
    vec3 airColor = mix(backdrop, uGlowColor, 0.12);
    outputColor = mix(backdrop, airColor, atmosphere);
    outputColor = mix(outputColor, fiberInk, fibers * 0.26);
  } else {
...
const GhostFibers = ({
  ...
  useEffect(() => {
    ...
    setColor(u.uLineColor as { value: Float32Array }, lineColor);
    setColor(u.uGlowColor as { value: Float32Array }, glowColor);
    (u.uSpeed as { value: number }).value = speed;
    (u.uScale as { value: number }).value = scale;
    (u.uRotation as { value: number }).value = rotation;
    (u.uRotationSpeed as { value: number }).value = rotationSpeed;
    (u.uLayers as { value: number }).value = Math.min(Math.max(Math.round(layers), 1), 10);
    (u.uWaveAmplitude as { value: number }).value = waveAmplitude;
    (u.uWaveFrequency as { value: number }).value = waveFrequency;
    (u.uWaveSpeed as { value: number }).value = waveSpeed;
    (u.uLayerSpeed as { value: number }).value = layerSpeed;
    (u.uTwist as { value: number }).value = twist;
    (u.uTwistFrequency as { value: number }).value = twistFrequency;
    (u.uTwistSpeed as { value: number }).value = twistSpeed;
    (u.uLineFrequency as { value: number }).value = lineFrequency;
    (u.uLineSpacing as { value: number }).value = lineSpacing;
    (u.uLineSharpness as { value: number }).value = lineSharpness;
    (u.uGlowFalloff as { value: number }).value = glowFalloff;
    (u.uGlowIntensity as { value: number }).value = glowIntensity;
    (u.uBrightness as { value: number }).value = brightness;
    (u.uBlueBoost as { value: number }).value = blueBoost;
    (u.uVignette as { value: number }).value = vignette;
    (u.uGrain as { value: number }).value = grain;
