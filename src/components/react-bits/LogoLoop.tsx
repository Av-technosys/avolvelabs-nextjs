...
type LogoLoopProps<T extends LogoLoopItem = LogoLoopItem> = {
  logos: T[];
  speed?: number;
  direction?: Direction;
  width?: number | string;
  ...
  renderItem?: (item: T, key: Key) => ReactNode;
  ariaLabel?: string;
  className?: string;
  style?: CSSProperties;
};
...
function LogoLoopComponent<T extends LogoLoopItem>({
  logos,
  speed = 120,
  direction = "left",
  width = "100%",
    ...
    (item: T, key: Key) => {
      ...
      const content = isNodeItem ? (
        ...
        <NextImage
          src={item.src}
          sizes={item.sizes}
          width={item.width ?? Math.ceil(logoHeight * 4)}
...
const LogoLoop = memo(LogoLoopComponent) as typeof LogoLoopComponent;
